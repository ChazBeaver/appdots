...
-- Theme Selector: Omarchy owns themes on Linux. The switcher is a desktop
-- menu; the FocusGained hook in omarchy_theme_apply reapplies the result.
vim.keymap.set("n", "<leader>tt", function()
  if vim.fn.executable("omarchy-theme-switcher") == 0 then
    vim.notify("omarchy-theme-switcher not found", vim.log.levels.WARN)
    return
  end
  vim.fn.jobstart({ "omarchy-theme-switcher" }, { detach = true })
end, { desc = "Omarchy theme picker" })
