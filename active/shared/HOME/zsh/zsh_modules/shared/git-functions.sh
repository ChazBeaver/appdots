...
grc() {
  ...
  if [[ -z "$msg" ]]; then
    echo "❌ Usage: grc [directory] \"commit message\""
    return 1
  fi

  for d in "$parent"/*(N/); do
    name="${d:t}"
    (
      cd "$d" || exit
      ...
      if [[ -e .git ]]; then
        ...
        echo "⏭️ [$name] skipped (not a git repo)"
      fi
    )
  done
}

_ad_register grc git/repos 'grc [DIRECTORY] "COMMIT MESSAGE"' 'Stage, commit, and push every child Git repository.' danger 'dirgcm gcmdir'
...
# ---------- gbr: fetch/prune, then report branch status ----------
gbr() {
  local base
  base="$(_ad_gb_base "$1")"
...
_ad_register gbr git/branch 'gbr [BASE]' 'Fetch and report active, merged, and upstream-gone local branches.' run
